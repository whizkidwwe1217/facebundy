...
public class UploadService extends IntentService {
    ...
    public static final String TOTAL_ITEMS_TO_UPLOAD = "total";
    public static final String TOTAL_ITEMS_UPLOADED = "uploadedItems";
    ...
    public static void startUpload(final UploadRequest task) throws IllegalArgumentException, MalformedURLException {
        ...
        if (task == null) {
            ...
            intent.putExtra(TOTAL, task.getTotal());
            intent.putExtra(CURRENT, task.getCurrent());
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent != null) {
            ...
            if (getActionUpload().equals(action)) {
                ...
                try {
                    createNotification();
                    handleFileUpload(uploadId, url, method, files, headers, parameters, total, current);
    ...
    private void handleFileUpload(final String uploadId, final String url, final String method,
                                  ...
                                  final ArrayList<NameValue> requestParameters, final int total, final int current) throws IOException {
        ...
        try {
            ...
            broadcastCompleted(uploadId, serverResponseCode, serverResponseMessage, filesToUpload.get(0).getFile().getAbsolutePath(), total, current);
    ...
    private void broadcastCompleted(final String uploadId, final int responseCode, final String responseMessage, final String tag, final int total, final int current) {
        ...
        intent.putExtra(TAG_FILENAME, tag);
        ...
        TimelogDataSource ds = TimelogDataSource.getInstance(getApplicationContext());
        try {
            ds.open();
            ds.beginTransaction();
            ds.deleteById(Integer.parseInt(uploadId));
        ...
        if(current >= total && total > 0 && GlobalConstants.getInstance().getAppEdition() != AppEditions.ExtractionAndMatching) {
